---
title: Eklentiler
description: Daemon Protocol'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Daemon Protocol'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Daemon Protocol'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
Daemon Protocol bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Daemon Protocol SDK istemcisi.
...
Daemon Protocol'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Daemon Protocol masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Daemon Protocol'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Daemon Protocol'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Daemon Protocol'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Daemon Protocol'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://daemonprotocol.com/docs/sdk) bakın.
