---
title: الويب
description: استخدام Daemon Protocol في متصفحك.
---

يمكن تشغيل Daemon Protocol كتطبيق ويب داخل متصفحك، ليمنحك تجربة البرمجة القوية بالذكاء الاصطناعي نفسها دون الحاجة إلى terminal.

![Daemon Protocol Web - جلسة جديدة](../../../assets/web/web-homepage-new-session.png)
...
يؤدي ذلك إلى تشغيل خادم محلي على `127.0.0.1` بمنفذ عشوائي متاح، ويفتح Daemon Protocol تلقائيا في المتصفح الافتراضي لديك.
...
افتراضيا، يختار Daemon Protocol منفذا متاحا. يمكنك تحديد منفذ:
...
افتراضيا، يرتبط الخادم بـ `127.0.0.1` (للاستخدام المحلي فقط). لجعل Daemon Protocol متاحا على شبكتك:

```bash
opencode web --hostname 0.0.0.0
```

عند استخدام `0.0.0.0` سيعرض Daemon Protocol كلا من عناوين الوصول المحلي وعناوين الشبكة:
...
بعد التشغيل، تتيح لك واجهة الويب الوصول إلى جلسات Daemon Protocol الخاصة بك.

### الجلسات

اعرض جلساتك وأدرها من الصفحة الرئيسية. يمكنك رؤية الجلسات النشطة وبدء جلسات جديدة.

![Daemon Protocol Web - جلسة نشطة](../../../assets/web/web-homepage-active-session.png)

### حالة الخادم

انقر على "See Servers" لعرض الخوادم المتصلة وحالتها.

![Daemon Protocol Web - See Servers (عرض الخوادم)](../../../assets/web/web-homepage-see-servers.png)
