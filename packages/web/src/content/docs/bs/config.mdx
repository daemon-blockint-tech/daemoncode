---
title: Konfiguracija
description: Korištenje Daemon Protocol JSON konfiguracije.
---

Možete konfigurirati Daemon Protocol koristeći JSON konfiguracijski fajl.

---

## Format

Daemon Protocol podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Postavite svoju globalnu Daemon Protocol konfiguraciju u `~/.config/opencode/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Kada se Daemon Protocol pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`daemonprotocol.com/config.json`**](https://daemonprotocol.com/config.json).

TUI konfiguracija koristi [**`daemonprotocol.com/tui.json`**](https://daemonprotocol.com/tui.json).
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Daemon Protocol server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Daemon Protocol konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Daemon Protocol pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Daemon Protocol će se vratiti na `"build"` s upozorenjem.
...
Daemon Protocol će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Daemon Protocol **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Daemon Protocol sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite Daemon Protocol da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
