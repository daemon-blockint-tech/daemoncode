...
Wenn Daemon Protocol Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Daemon Protocol Desktop startet im Hintergrund einen lokalen Daemon Protocol-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Daemon Protocol`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Daemon Protocol kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Daemon Protocol ihn neu aufbauen kann.

1. Beende Daemon Protocol Desktop komplett.
...
3. Starte Daemon Protocol Desktop neu.

---

### Server-Verbindungsprobleme beheben

Daemon Protocol Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt Daemon Protocol Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Daemon Protocol-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Daemon Protocol Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Daemon Protocol in den OS-Einstellungen aktiviert sind, und
...
1. Beende Daemon Protocol Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Daemon Protocol Desktop):
...
Wenn du Probleme mit Daemon Protocol hast:
