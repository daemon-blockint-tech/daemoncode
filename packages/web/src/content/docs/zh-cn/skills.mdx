...
代理技能让 Daemon Protocol 能够从你的仓库或主目录中发现可复用的指令。
...
对于项目本地路径，Daemon Protocol 会从当前工作目录向上遍历，直到到达 git 工作树根目录。
...
Daemon Protocol 会在 `skill` 工具描述中列出可用技能。
每个条目包含技能名称和描述：
