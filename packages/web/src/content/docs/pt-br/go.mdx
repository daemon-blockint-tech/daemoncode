...
O Daemon Protocol Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Daemon Protocol. Você assina o Daemon Protocol Go e
obtém a sua chave de API. Ele é **totalmente opcional** e você não precisa usá-lo para
usar o Daemon Protocol.
...
O Daemon Protocol Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Daemon Protocol Go funciona como qualquer outro provedor no Daemon Protocol.

1. Você entra no **<a href={console}>Daemon Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Daemon Protocol Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Daemon Protocol Go.
:::
...
O Daemon Protocol Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Daemon Protocol
...
Criamos o Daemon Protocol Go para:
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Daemon Protocol também.
