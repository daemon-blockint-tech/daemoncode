...
import { InstallationLocal, InstallationVersion } from "@daemon-protocol/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@daemon-protocol/core/v1/config/console-state"
import { FSUtil } from "@daemon-protocol/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@daemon-protocol/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@daemon-protocol/core/v1/config/config"
import { RemoteAuthError } from "@daemon-protocol/core/v1/config/error"
import { ConfigPermissionV1 } from "@daemon-protocol/core/v1/config/permission"
import { ConfigPluginV1 } from "@daemon-protocol/core/v1/config/plugin"
...
import { Npm } from "@daemon-protocol/core/npm"
...
function globalConfigFile() {
  const candidates = ["daemoncode.jsonc", "daemoncode.json", "opencode.jsonc", "opencode.json", "config.json"].map(
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://daemonprotocol.com/config.json"
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://daemonprotocol.com/config.json",')
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://daemonprotocol.com/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "daemoncode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "daemoncode.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://daemonprotocol.com/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://daemonprotocol.com/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("daemoncode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".daemoncode") || dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const source of ConfigPaths.fileInDirectory(dir, "daemoncode")) {
        ...
        if (existsSync(managedDir)) {
          for (const source of ConfigPaths.fileInDirectory(managedDir, "daemoncode")) {
            if (!existsSync(source)) continue
