...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Daemon Protocol.
...
Voici plusieurs modèles qui fonctionnent bien avec Daemon Protocol, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Daemon Protocol.
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Daemon Zen](/docs/zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Daemon Protocol est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Daemon Protocol est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
Lorsque Daemon Protocol démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Daemon Protocol.
