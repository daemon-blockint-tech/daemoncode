# Contributing to Daemon Protocol

We want to make it easy for you to contribute to Daemon Protocol. Here are the most common type of changes that get merged:
...
By default, `bun dev` runs Daemon Protocol in the `packages/opencode` directory. To run it against a different directory or repository:

```bash
bun dev <directory>
```

To run Daemon Protocol in the root of the opencode repo itself:
...
- Core pieces:
  - `packages/opencode`: Daemon Protocol core business logic & server.
...
To start the Daemon Protocol headless API server:
...
1. **First, start the Daemon Protocol server** (see [Running the API Server](#running-the-api-server) section above)
...
The most reliable way to debug Daemon Protocol is to run it manually in a terminal via `bun run --inspect=<url> dev ...` and attach
your debugger via that URL. Other methods can result in breakpoints being mapped incorrectly, at least in VSCode (YMMV).

Caveats:

- If you want to run the Daemon Protocol TUI and have breakpoints triggered in the server code, you might need to run `bun dev spawn` instead of
...
- The same problem arises when running Daemon Protocol in the VSCode `JavaScript Debug Terminal`
...
For net-new functionality, start with a design conversation. Open an issue describing the problem, your proposed approach (optional), and why it belongs in Daemon Protocol. The core team will help decide whether it should move forward; please wait for that approval instead of opening a feature PR directly.
