...
Du kan legge til egendefinerte kommandoer gjennom Daemon Protocol-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i Daemon Protocol [config](/docs/config):
...
Daemon Protocol inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
