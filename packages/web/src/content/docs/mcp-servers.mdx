...
You can add external tools to Daemon Protocol using the _Model Context Protocol_, or MCP. Daemon Protocol supports both local and remote servers.
...
You can define MCP servers in your [Daemon Protocol Config](https://daemonprotocol.com/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Daemon Protocol automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Daemon Protocol will:
...
If the server requires authentication, Daemon Protocol will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Daemon Protocol will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in Daemon Protocol, alongside built-in tools. So you can manage them through the Daemon Protocol config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Daemon Protocol to your Sentry account.
