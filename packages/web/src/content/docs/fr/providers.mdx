---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Daemon Protocol.
---

import config from "../../../../config.mjs"
export const console = config.console

Daemon Protocol utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Daemon Protocol.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Daemon Protocol
configuration.
...
Daemon Zen est une liste de modèles fournis par l'équipe Daemon Protocol qui ont été
testé et vérifié pour fonctionner correctement avec Daemon Protocol. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Daemon Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez opencode et dirigez-vous vers [daemonprotocol.com/auth](https://daemonprotocol.com/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Daemon Protocol et son utilisation est totalement facultative.

---

## Daemon Protocol Go

Daemon Protocol Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Daemon Protocol qui ont été
testé et vérifié pour fonctionner correctement avec Daemon Protocol.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Daemon Protocol Go` et rendez-vous sur [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
Pour utiliser Amazon Bedrock avec Daemon Protocol :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Daemon Protocol n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
3.  Nom : `Daemon Protocol`, Portées : `api`
...
Daemon Protocol utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Daemon Protocol
...
Pour utiliser Google Vertex AI avec Daemon Protocol :
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation Daemon Protocol en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Daemon Protocol répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour Daemon Protocol. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Daemon Protocol :
...
3. Copiez la clé API à utiliser dans Daemon Protocol.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Daemon Protocol, vous devez extraire les informations du modèle localement :
...
Daemon Zen est une liste de modèles testés et vérifiés fournie par l'équipe Daemon Protocol. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Daemon Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Daemon Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Daemon Protocol API.
...
Les champs `limit` permettent à Daemon Protocol de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
