---
title: Providers
description: Using any LLM provider in Daemon Protocol.
---

import config from "../../../config.mjs"
export const console = config.console

Daemon Protocol uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Daemon Protocol config.
...
You can customize the providers through the `provider` section in your Daemon Protocol
config.
...
Daemon Zen is a list of models provided by the Daemon Protocol team that have been
tested and verified to work well with Daemon Protocol. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Daemon Zen.
:::

1. Run the `/connect` command in the TUI, select `Daemon Zen`, and head to [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
It works like any other provider in Daemon Protocol and is completely optional to use.

---

## Daemon Protocol Go

Daemon Protocol Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Daemon Protocol team that have been
tested and verified to work well with Daemon Protocol.

1. Run the `/connect` command in the TUI, select `Daemon Protocol Go`, and head to [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
To use Amazon Bedrock with Daemon Protocol:
...
Daemon Protocol. Anthropic explicitly prohibits this.

Previous versions of Daemon Protocol came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Daemon Protocol with zero setup:
...
Daemon Protocol supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Daemon Protocol uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Daemon Protocol. Sign in and approve.

   :::note
   Daemon Protocol requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Daemon Protocol surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Daemon Protocol forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Daemon Protocol is experimental. Features, configuration, and
behavior may change in future releases.
:::

Daemon Protocol integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   3. Name: `Daemon Protocol`, Scopes: `api`
...
Daemon Protocol uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Daemon Protocol
...
When a `duo-workflow-*` model is selected, Daemon Protocol will:
...
4. Route tool execution requests through Daemon Protocol's permission-gated tool system
...
To use Google Vertex AI with Daemon Protocol:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Daemon Protocol conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Daemon Protocol conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for Daemon Protocol. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with Daemon Protocol:
...
3. Copy the API key for use in Daemon Protocol.
...
6. **Important**: Before using cloud models in Daemon Protocol, you must pull the model information locally:
...
Daemon Zen is a list of tested and verified models provided by the Daemon Protocol team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Daemon Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Daemon Zen**.

   ```txt
   /connect
   ```

3. Enter your Daemon Protocol API key.
...
5. Complete sign-in in the browser that opens. Daemon Protocol captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Daemon Protocol opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Daemon Protocol refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Daemon Protocol is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Daemon Protocol — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Daemon Protocol prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Daemon Protocol polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
The `limit` fields allow Daemon Protocol to understand how much context you have left. Standard providers pull these from models.dev automatically.
