...
Es gibt zwei Arten von Agenten in Daemon Protocol; Primäragenten und Subagenten.
...
Daemon Protocol verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Wenn keine Temperatur angegeben ist, verwendet Daemon Protocol modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Daemon Protocol-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Daemon Protocol-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Daemon Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/daemon-blockint-tech/daemoncode).
:::
