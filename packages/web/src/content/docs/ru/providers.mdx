...
Daemon Zen — это список моделей, предоставленный командой opencode, которые были
протестировано и проверено на хорошую работу с opencode. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с Daemon Zen.
:::

1. Запустите команду `/connect` в TUI, выберите `Daemon Zen` и перейдите по адресу [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
Daemon Protocol Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой opencode, которые были
протестированы и проверены на хорошую работу с opencode.

1. Запустите команду `/connect` в TUI, выберите `Daemon Protocol Go` и перейдите по адресу [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `Daemon Protocol`, Области применения: `api`
...
Daemon Zen — это список протестированных и проверенных моделей, предоставленный командой opencode. [Подробнее](/docs/zen).

1. Войдите в систему **<a href={console}>Daemon Zen</a>** и нажмите **Создать ключ API**.

2. Запустите команду `/connect` и найдите **Daemon Zen**.
