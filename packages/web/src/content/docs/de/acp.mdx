...
description: Verwenden Sie Daemon Protocol in jedem ACP-kompatiblen Editor.
---

Daemon Protocol unterstützt [Agent Client Protocol](https://agentclientprotocol.com) oder (ACP), sodass Sie es direkt in kompatiblen Editoren und IDEs verwenden können.
...
Um Daemon Protocol über ACP zu verwenden, konfigurieren Sie Ihren Editor für die Ausführung des Befehls `opencode acp`.

Der Befehl startet Daemon Protocol als ACP-kompatiblen Unterprozess, der über stdio über JSON-RPC mit Ihrem Editor kommuniziert.
...
Um es zu öffnen, verwenden Sie den neuen Agenten „Daemon Protocol“ in der AI Chat Agent Selector.
...
Um Daemon Protocol als ACP-Agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) zu verwenden, fügen Sie Folgendes zu Ihrer Neovim-Konfiguration hinzu:
...
Diese Konfiguration richtet CodeCompanion so ein, dass Daemon Protocol als ACP-Agent für den Chat verwendet wird.
...
Daemon Protocol funktioniert über ACP genauso wie im Terminal. Alle Funktionen werden unterstützt:
...
- MCP-Server, die in Ihrer Daemon Protocol-Konfiguration konfiguriert sind
