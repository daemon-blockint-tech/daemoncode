...
Da biste otklonili probleme s Daemon Protocol, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Daemon Protocol pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
Daemon Protocol Desktop pokreće lokalni Daemon Protocol server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Daemon Protocol` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
Daemon Protocol također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Daemon Protocol mogao ponovo izgraditi.

1. Potpuno zatvorite Daemon Protocol Desktop.
...
3. Ponovo pokrenite Daemon Protocol Desktop.

---

### Rješavanje problema sa vezom na serveru

Daemon Protocol Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Na Windows-u, Daemon Protocol Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Daemon Protocol karakteristikama.

---

### Obavještenja se ne prikazuju

Daemon Protocol Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Daemon Protocol u postavkama vašeg OS-a, i
...
1. Zatvorite Daemon Protocol Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Daemon Protocol Desktop aplikacije):
...
Ako imate problema s Daemon Protocol:

1. **Prijavite probleme na GitHub**

   Najbolji način da prijavite greške ili zatražite funkcije je putem našeg GitHub spremišta:

   [**github.com/daemon-blockint-tech/daemoncode/issues**](https://github.com/daemon-blockint-tech/daemoncode/issues)
...
2. **Pridružite se našem Discordu**
   ...
   [**daemonprotocol.com/discord**](https://daemonprotocol.com/discord)
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Daemon Protocol dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite Daemon Protocol da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Daemon Protocol da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Daemon Protocol će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
