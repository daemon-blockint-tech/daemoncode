...
Move the canonical Daemon Protocol terminal application from
...
```text
packages/opencode ---\
                      > @daemon-protocol/tui -> @daemon-protocol/sdk
packages/cli --------/
```
...
`packages/cli`, or `@daemon-protocol/core`.

The SDK is the TUI's Daemon Protocol boundary. Missing backend data or operations must
...
- Do not replace private `packages/opencode` imports with `@daemon-protocol/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@daemon-protocol/tui`.

## Ownership Boundary

### `@daemon-protocol/tui` Owns
...
- Daemon Protocol domain data displayed by the TUI
...
Status: Completed. The private `@daemon-protocol/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@daemon-protocol/tui`.
...
- The package has no dependency on `opencode`, `@daemon-protocol/cli`, or
...
primitives now live in `@daemon-protocol/tui`. The legacy host consumes explicit
...
and build inputs through `@daemon-protocol/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Daemon Protocol globals or process state;
...
Replace process-global Daemon Protocol state with resolved TUI inputs.
...
`@daemon-protocol/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@daemon-protocol/tui`. Configured references resolve through the new
...
adapters rather than Daemon Protocol domain implementations.

Make the SDK the only Daemon Protocol domain boundary used by the TUI.
...
- Domain-facing TUI code imports Daemon Protocol data and operations only from
  `@daemon-protocol/sdk`.
...
`@daemon-protocol/tui`. The legacy host injects a narrow plugin host that retains
...
public `@daemon-protocol/tui` root while retaining worker/server/config/plugin and
...
- Change those adapters to load config, create transport inputs, and call the
  public `@daemon-protocol/tui` API.
...
- Narrow `@daemon-protocol/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@daemon-protocol/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Daemon Protocol domain state.
...
rg '@daemon-protocol/core|packages/opencode|packages/cli' packages/tui
