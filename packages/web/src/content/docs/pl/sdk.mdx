---
title: SDK
description: Typowany klient JS dla serwera Daemon Protocol.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Pakiet SDK JS/TS Daemon Protocol zapewnia klienta z bezpiecznym typowaniem (type-safe) do interakcji z serwerem.
Użyj go do budowania integracji i programowej kontroli Daemon Protocol.
...
```bash
npm install @daemon-protocol/sdk
```
...
Utwórz instancję Daemon Protocol:

```javascript
import { createDaemonCode } from "@daemon-protocol/sdk"

const { client } = await createDaemonCode()
```
...
Jeśli masz już działającą instancję Daemon Protocol, możesz utworzyć instancję klienta, aby się z nią połączyć:
...
```typescript
import type { Session, Message, Part } from "@daemon-protocol/sdk"
```
