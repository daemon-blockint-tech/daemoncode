...
Sie können externe Tools zu Daemon Protocol hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Daemon Protocol unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Daemon Protocol Config](https://daemonprotocol.com/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Daemon Protocol übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Daemon Protocol Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Daemon Protocol beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `opencode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Daemon Protocol die Token sicher in `~/.local/share/opencode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Daemon Protocol verfügbar. Sie können sie also wie jedes andere Tool über die Daemon Protocol-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Daemon Protocol mit Ihrem Sentry-Konto zu verbinden.
