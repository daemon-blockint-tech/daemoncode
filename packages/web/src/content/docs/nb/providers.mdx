---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Daemon Protocol.
---

import config from "../../../../config.mjs"
export const console = config.console

Daemon Protocol bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Daemon Protocol-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i Daemon Protocol
konfig.
...
Daemon Zen er en liste over modeller levert av Daemon Protocol-teamet som har vært
testet og verifisert for å fungere godt med Daemon Protocol. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Daemon Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Daemon Zen` og gå til [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
Det fungerer som alle andre leverandører i Daemon Protocol og er helt valgfritt å bruke.

---

## Daemon Protocol Go

Daemon Protocol Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Daemon Protocol-teamet som har vært
testet og verifisert for å fungere godt med Daemon Protocol.

1. Kjør kommandoen `/connect` i TUI, velg `Daemon Protocol Go`, og gå til [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
Slik bruker du Amazon Bedrock med Daemon Protocol:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Daemon Protocol:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Daemon Protocol skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Daemon Protocol-konfigurasjonen.
...
2. Velg autentiseringsmetoden din:
   ...
   3. Navn: `Daemon Protocol`, omfang: `api`
...
for å lagre token i Daemon Protocol auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Daemon Protocol bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Daemon Protocol
...
Slik bruker du GitHub Copilot-abonnementet med Daemon Protocol:
...
Slik bruker du Google Vertex AI med Daemon Protocol:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Daemon Protocol.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Daemon Protocol, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Daemon Protocol-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Daemon Protocol-samtale oppført som en separat økt.
...
Du kan konfigurere Daemon Protocol for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Daemon Protocol for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Daemon Protocol for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Daemon Protocol. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Daemon Protocol:
...
3. Kopier API-nøkkelen for bruk i Daemon Protocol.
...
6. **Viktig**: Før du bruker skymodeller i Daemon Protocol, må du hente modellinformasjonen lokalt:
...
Daemon Zen er en liste over testede og verifiserte modeller levert av Daemon Protocol-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Daemon Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Daemon Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Daemon Protocol API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Daemon Protocol-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Daemon Protocol-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Daemon Protocol:
...
Du kan også tilpasse modeller gjennom Daemon Protocol-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Daemon Protocol. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar Daemon Protocol forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk Daemon Protocol-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Daemon Protocol-konfigurasjonen.
