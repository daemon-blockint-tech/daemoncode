...
For at fejlfinde problemer med Daemon Protocol, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Daemon Protocol Desktop kører en lokal Daemon Protocol-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Daemon Protocol`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
Daemon Protocol kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Daemon Protocol kan genopbygge den.

1. Afslut Daemon Protocol Desktop helt.
2. Slet cache-mappen:
...
3. Start Daemon Protocol Desktop på ny.

---

### Løs problemer med serverforbindelse

Daemon Protocol Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
På Windows kræver Daemon Protocol Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Daemon Protocol.

---

### Meddelelser vises ikke

Daemon Protocol Desktop viser kun systemvarsler når:

- varsler er aktiveret for Daemon Protocol i dine OS-indstillinger, og
...
1. Afslut Daemon Protocol Desktop.
2. Find og slet disse filer (de findes i Daemon Protocol Desktop-appens datamappe):
...
Hvis du oplever problemer med Daemon Protocol:
...
### Daemon Protocol vil ikke starte
