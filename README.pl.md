...
# Menedżery pakietów
npm i -g daemoncode@latest        # albo bun/pnpm/yarn
scoop install daemoncode             # Windows
choco install daemoncode             # Windows
brew install daemon-blockint-tech/tap/daemoncode # macOS i Linux (polecane, zawsze aktualne)
brew install daemoncode              # macOS i Linux (oficjalna formuła brew, rzadziej aktualizowana)
sudo pacman -S daemoncode            # Arch Linux (Stable)
paru -S daemoncode-bin               # Arch Linux (Latest from AUR)
mise use -g daemoncode               # dowolny system
nix run nixpkgs#daemoncode           # lub github:daemon-blockint-tech/daemoncode dla najnowszej gałęzi dev
```
...
Daemon Protocol jest także dostępny jako aplikacja desktopowa. Pobierz ją bezpośrednio ze strony [releases](https://github.com/daemon-blockint-tech/daemoncode/releases) lub z [daemonprotocol.com/download](https://daemonprotocol.com/download).
...
Daemon Protocol zawiera dwóch wbudowanych agentów, między którymi możesz przełączać się klawiszem `Tab`.
...
Dowiedz się więcej o [agents](https://daemonprotocol.com/docs/agents).

### Dokumentacja

Więcej informacji o konfiguracji Daemon Protocol znajdziesz w [**dokumentacji**](https://daemonprotocol.com/docs).

### Współtworzenie

Jeśli chcesz współtworzyć Daemon Protocol, przeczytaj [contributing docs](./CONTRIBUTING.md) przed wysłaniem pull requesta.

### Budowanie na Daemon Protocol

Jeśli pracujesz nad projektem związanym z Daemon Protocol i używasz "opencode" jako części nazwy (na przykład "opencode-dashboard" lub "opencode-mobile"), dodaj proszę notatkę do swojego README, aby wyjaśnić, że projekt nie jest tworzony przez zespół Daemon Protocol i nie jest z nami w żaden sposób powiązany.
