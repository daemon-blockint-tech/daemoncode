...
Der er to typer agenter i Daemon Protocol; primære agenter og subagenter.
...
Daemon Protocol leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Daemon Protocol leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

Daemon Protocol leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i Daemon Protocol's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Hvis ingen temperatur er angivet, bruger Daemon Protocol modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Daemon Protocol-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Daemon Protocol-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Daemon Zen](/docs/zen), vil du bruge `opencode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/daemon-blockint-tech/daemoncode).
:::
