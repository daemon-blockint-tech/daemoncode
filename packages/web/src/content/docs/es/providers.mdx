---
title: Proveedores
description: Usando cualquier proveedor LLM en Daemon Protocol.
---

import config from "../../../../config.mjs"
export const console = config.console

Daemon Protocol usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Daemon Protocol.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Daemon Protocol
configuración.
...
Daemon Zen es una lista de modelos proporcionados por el equipo Daemon Protocol que han sido
probado y verificado para funcionar bien con Daemon Protocol. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Daemon Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione opencode y diríjase a [daemonprotocol.com/auth](https://daemonprotocol.com/auth).
...
Funciona como cualquier otro proveedor en Daemon Protocol y su uso es completamente opcional.

---

## Daemon Protocol Go

Daemon Protocol Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Daemon Protocol que han sido
probado y verificado para funcionar bien con Daemon Protocol.

1. Ejecute el comando `/connect` en TUI, seleccione `Daemon Protocol Go` y diríjase a [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
Para usar Amazon Bedrock con Daemon Protocol:
...
:::info
El uso de su suscripción Claude Pro/Max en Daemon Protocol no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Daemon Protocol`, Alcances: `api`
...
Daemon Protocol utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Daemon Protocol
...
Para utilizar Google Vertex AI con Daemon Protocol:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Daemon Protocol como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Daemon Protocol enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para Daemon Protocol. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Daemon Protocol:
...
3. Copie la clave API para usarla en Daemon Protocol.
...
6. **Importante**: Antes de usar modelos en la nube en Daemon Protocol, debe obtener la información del modelo localmente:
...
Daemon Zen es una lista de modelos probados y verificados proporcionada por el equipo Daemon Protocol. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Daemon Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Daemon Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Daemon Protocol API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Daemon Protocol:
...
Los campos `limit` le permiten a Daemon Protocol comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
