---
title: TUI
description: Daemon Protocol terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Daemon Protocol, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Daemon Protocol'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Daemon Protocol TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Daemon Protocol'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Daemon Protocol'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
- `leader_timeout` - Daemon Protocol'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
