...
Daemon Protocol Go هو اشتراك منخفض التكلفة — **$5 للشهر الأول**، ثم **$10/شهريًا** — يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.

يعمل Go مثل أي مزود آخر في Daemon Protocol. تشترك في Daemon Protocol Go وتحصل على مفتاح API الخاص بك. وهو **اختياري تمامًا**، ولا تحتاج إلى استخدامه لاستخدام Daemon Protocol.
...
:::tip
اختبرنا مجموعة مختارة من النماذج والمزودين الذين يعملون جيدًا مع Daemon Protocol.
:::
...
يمنحك Daemon Protocol Go الوصول إلى هذه النماذج مقابل **$5 للشهر الأول**، ثم **$10/شهريًا**.

---

## كيف يعمل

يعمل Daemon Protocol Go مثل أي مزود آخر في Daemon Protocol.

1. تسجّل الدخول إلى **<a href={console}>Daemon Zen</a>**، وتشترك في Go، ثم تنسخ مفتاح API الخاص بك.
2. تشغّل الأمر `/connect` في TUI، وتختار `Daemon Protocol Go`، ثم تلصق مفتاح API الخاص بك.
3. شغّل `/models` في TUI لرؤية قائمة النماذج المتاحة عبر Go.

:::note
يمكن لعضو واحد فقط في كل workspace الاشتراك في Daemon Protocol Go.
:::
...
يتضمن Daemon Protocol Go الحدود التالية:
...
يستخدم [model id](/docs/config/#models) في إعدادات Daemon Protocol لديك التنسيق `opencode-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K2.7 Code، ستستخدم `opencode-go/kimi-k2.7-code` في إعداداتك.
...
أنشأنا Daemon Protocol Go من أجل:
...
4. **عدم فرض قيود ارتباط** من خلال السماح لك أيضًا باستخدام أي مزود آخر مع Daemon Protocol.
