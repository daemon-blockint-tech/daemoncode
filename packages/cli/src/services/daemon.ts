...
import { createDaemonCodeClient } from "@daemon-protocol/sdk/v2/client"
import { ServerAuth } from "@daemon-protocol/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createDaemonCodeClient>, unknown>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createDaemonCodeClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createDaemonCodeClient({ baseUrl: connection.url, headers: connection.headers })
    })
