...
Daemon Protocol Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en Daemon Protocol. Te suscribes a Daemon Protocol Go y
obtienes tu API key. Es **completamente opcional** y no necesitas usarlo para
usar Daemon Protocol.
...
Daemon Protocol Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

Daemon Protocol Go funciona como cualquier otro proveedor en Daemon Protocol.

1. Inicias sesión en **<a href={console}>Daemon Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Daemon Protocol Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Daemon Protocol Go.
:::
...
Daemon Protocol Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de Daemon Protocol
...
Creamos Daemon Protocol Go para:
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Daemon Protocol.
