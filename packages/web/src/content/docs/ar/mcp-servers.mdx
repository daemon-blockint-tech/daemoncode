...
يمكنك إضافة أدوات خارجية إلى Daemon Protocol باستخدام _Model Context Protocol_، أو MCP. يدعم Daemon Protocol خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Daemon Protocol](https://daemonprotocol.com/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يتولى Daemon Protocol تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Daemon Protocol بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Daemon Protocol المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `opencode mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Daemon Protocol الرموز بشكل آمن في `~/.local/share/opencode/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Daemon Protocol إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Daemon Protocol مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Daemon Protocol بحساب Sentry الخاص بك.
