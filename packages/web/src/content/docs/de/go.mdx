...
Daemon Protocol Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Daemon Protocol. Du abonnierst Daemon Protocol Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Daemon Protocol zu verwenden.
...
Daemon Protocol Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Daemon Protocol Go funktioniert wie jeder andere Provider in Daemon Protocol.

1. Du meldest dich bei **<a href={console}>Daemon Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Daemon Protocol Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Daemon Protocol Go abonnieren.
:::
...
Daemon Protocol Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Daemon Protocol Config verwendet das Format `opencode-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `opencode-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben Daemon Protocol Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Daemon Protocol zu nutzen.
