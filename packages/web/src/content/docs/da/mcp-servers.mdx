...
Du kan tilføje eksterne værktøjer til Daemon Protocol ved hjælp af _Model Context Protocol_ eller MCP. Daemon Protocol understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Daemon Protocol Config](https://daemonprotocol.com/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Daemon Protocol håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Daemon Protocol:
...
Hvis serveren kræver godkendelse, vil Daemon Protocol bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Daemon Protocol opbevare tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Daemon Protocol-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Daemon Protocol til din Sentry-konto.
