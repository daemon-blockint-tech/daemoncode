...
Možete dodati vanjske alate u Daemon Protocol koristeći _Model Context Protocol_, ili MCP. Daemon Protocol podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Daemon Protocol Config](https://daemonprotocol.com/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Daemon Protocol automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Daemon Protocol će:
...
Ako server zahtijeva autentifikaciju, Daemon Protocol će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Daemon Protocol sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Daemon Protocol, zajedno s ugrađenim alatima. Možete njima upravljati kroz Daemon Protocol konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Daemon Protocol sa vašim Sentry nalogom.
