...
Per diagnosticare problemi con Daemon Protocol, inizia controllando i log e i dati locali che salva su disco.
...
Daemon Protocol Desktop esegue in background un server locale di Daemon Protocol (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Daemon Protocol` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
Daemon Protocol puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Daemon Protocol possa ricostruirla.

1. Chiudi completamente Daemon Protocol Desktop.
...
3. Riavvia Daemon Protocol Desktop.

---

### Risolvi problemi di connessione al server

Daemon Protocol Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, Daemon Protocol Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Daemon Protocol.

---

### Notifiche non visualizzate

Daemon Protocol Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Daemon Protocol nelle impostazioni del sistema operativo, e
...
1. Chiudi Daemon Protocol Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Daemon Protocol Desktop):
...
Se riscontri problemi con Daemon Protocol:
