---
title: Supporto ACP
description: Usa Daemon Protocol in qualunque editor compatibile con ACP.
---

Daemon Protocol supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Daemon Protocol via ACP, configura l'editor in modo che esegua il comando `opencode acp`.

Il comando avvia Daemon Protocol come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Daemon Protocol' nel selettore agenti di AI Chat.
...
Per usare Daemon Protocol come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Daemon Protocol come agente ACP per la chat.
...
Daemon Protocol funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Daemon Protocol
