...
# Paketmanager
npm i -g daemoncode@latest        # oder bun/pnpm/yarn
scoop install daemoncode             # Windows
choco install daemoncode             # Windows
brew install daemon-blockint-tech/tap/daemoncode # macOS und Linux (empfohlen, immer aktuell)
brew install daemoncode              # macOS und Linux (offizielle Brew-Formula, seltener aktualisiert)
sudo pacman -S daemoncode            # Arch Linux (Stable)
paru -S daemoncode-bin               # Arch Linux (Latest from AUR)
mise use -g daemoncode               # jedes Betriebssystem
nix run nixpkgs#daemoncode           # oder github:daemon-blockint-tech/daemoncode für den neuesten dev-Branch
```
...
Daemon Protocol ist auch als Desktop-Anwendung verfügbar. Lade sie direkt von der [Releases-Seite](https://github.com/daemon-blockint-tech/daemoncode/releases) oder [daemonprotocol.com/download](https://daemonprotocol.com/download) herunter.
...
Daemon Protocol enthält zwei eingebaute Agents, zwischen denen du mit der `Tab`-Taste wechseln kannst.
...
Mehr dazu unter [Agents](https://daemonprotocol.com/docs/agents).

### Dokumentation

Mehr Infos zur Konfiguration von Daemon Protocol findest du in unseren [**Docs**](https://daemonprotocol.com/docs).

### Beitragen

Wenn du zu Daemon Protocol beitragen möchtest, lies bitte unsere [Contributing Docs](./CONTRIBUTING.md), bevor du einen Pull Request einreichst.

### Auf Daemon Protocol aufbauen

Wenn du an einem Projekt arbeitest, das mit Daemon Protocol zusammenhängt und "opencode" als Teil seines Namens verwendet (z.B. "opencode-dashboard" oder "opencode-mobile"), füge bitte einen Hinweis in deine README ein, dass es nicht vom Daemon Protocol-Team gebaut wird und nicht in irgendeiner Weise mit uns verbunden ist.
