---
title: 設定
description: Daemon Protocol JSON 設定を使用します。
---

JSON 設定ファイルを使用して Daemon Protocol を構成できます。

---

## 形式

Daemon Protocol は、**JSON** と **JSONC** (コメント付きの JSON) 形式の両方をサポートしています。
...
グローバル Daemon Protocol 設定を `~/.config/opencode/opencode.json` に配置します。プロバイダー、モデル、権限などのユーザー全体の設定にはグローバル設定を使用します。
...
Daemon Protocol が起動すると、現在のディレクトリで設定ファイルを検索するか、最も近い Git ディレクトリまで移動します。
...
サーバー/ランタイム構成スキーマは [**`daemonprotocol.com/config.json`**](https://daemonprotocol.com/config.json) で定義されています。

TUI 設定は [**`daemonprotocol.com/tui.json`**](https://daemonprotocol.com/tui.json) を使用します。
...
- `mdns` - mDNS サービス検出を有効にします。これにより、ネットワーク上の他のデバイスが Daemon Protocol サーバーを検出できるようになります。
...
`provider`、`model`、および `small_model` オプションを使用して、Daemon Protocol 設定で使用するプロバイダーとモデルを構成できます。
...
`small_model` オプションは、タイトルの生成などの軽量タスク用に個別のモデルを構成します。デフォルトでは、Daemon Protocol は、プロバイダーから安価なモデルが入手可能な場合は、より安価なモデルを使用しようとします。そうでない場合は、メインモデルにフォールバックします。
...
デフォルトのエージェントはプライマリエージェントである必要があります (サブエージェントではありません)。これは、`"build"` や `"plan"` のような組み込みエージェント、または定義した[カスタムエージェント](/docs/agents) にすることができます。指定されたエージェントが存在しないか、サブエージェントである場合、Daemon Protocol は警告とともに `"build"` にフォールバックします。
...
Daemon Protocol は起動時に新しいアップデートを自動的にダウンロードします。 `autoupdate` オプションを使用してこれを無効にできます。
...
[Plugins](/docs/plugins) は、カスタムツール、フック、統合を使用して Daemon Protocol を拡張します。
...
これは、Daemon Protocol を 1 つずつ無効にするのではなく、特定のプロバイダーのみを使用するように制限したい場合に便利です。
