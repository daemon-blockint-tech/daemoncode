...
Daemon Zen to lista modeli dostarczonych przez zespół opencode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opencode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Daemon Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Daemon Zen` i przejdź do [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
Daemon Protocol Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół opencode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opencode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Daemon Protocol Go` i przejdź do [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
Daemon Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opencode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Daemon Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Daemon Zen**.
