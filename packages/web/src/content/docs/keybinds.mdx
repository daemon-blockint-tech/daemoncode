...
Daemon Protocol has a list of keybinds that you can customize through `tui.json`.
...
Daemon Protocol uses a `leader` key for many keybinds. This avoids conflicts in your terminal.
...
`leader_timeout` controls how long Daemon Protocol waits for the next key after the leader key. It defaults to `2000` milliseconds.
...
The Daemon Protocol desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `opencode.json`.
