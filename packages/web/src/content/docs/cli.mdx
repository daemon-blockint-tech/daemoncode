---
title: CLI
description: Daemon Protocol CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The Daemon Protocol CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
opencode
```

But it also accepts commands as documented on this page. This allows you to interact with Daemon Protocol programmatically.
...
Start the Daemon Protocol terminal user interface.
...
The Daemon Protocol CLI also has the following commands.

---

### agent

Manage agents for Daemon Protocol.
...
Attach a terminal to an already running Daemon Protocol backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote Daemon Protocol backend. For example:
...
Daemon Protocol is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

When Daemon Protocol starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Daemon Protocol.
...
Start a headless Daemon Protocol server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.
...
Manage Daemon Protocol sessions.
...
List all Daemon Protocol sessions.
...
Delete an Daemon Protocol session.
...
Show token usage and cost statistics for your Daemon Protocol sessions.
...
Import session data from a JSON file or Daemon Protocol share URL.

```bash
opencode import <file>
```

You can import from a local file or an Daemon Protocol share URL.
...
Start a headless Daemon Protocol server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access Daemon Protocol through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Fetch and checkout a GitHub PR branch, then run Daemon Protocol.
...
Uninstall Daemon Protocol and remove all related files.
...
Daemon Protocol can be configured using environment variables.
