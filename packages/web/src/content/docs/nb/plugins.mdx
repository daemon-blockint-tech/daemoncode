...
description: Skriv dine egne programtillegg for å utvide Daemon Protocol.
---

Plugins lar deg utvide Daemon Protocol ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Daemon Protocol.
...
Daemon Protocol kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Daemon Protocol SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Daemon Protocol.
...
:::note
Hvis du bruker Daemon Protocol-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Daemon Protocol fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Daemon Protocol:
...
`tool`-hjelperen lager et tilpasset verktøy som Daemon Protocol kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Daemon Protocol sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://daemonprotocol.com/docs/sdk) for detaljer.
