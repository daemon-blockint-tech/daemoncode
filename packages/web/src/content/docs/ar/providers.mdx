---
title: المزوّدون
description: استخدام أي موفّر LLM في Daemon Protocol.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Daemon Protocol ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Daemon Protocol.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Daemon Protocol.
...
Daemon Zen هي قائمة نماذج يوفّرها فريق Daemon Protocol وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Daemon Protocol. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Daemon Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [daemonprotocol.com/auth](https://daemonprotocol.com/auth).
...
يعمل مثل أي مزوّد آخر في Daemon Protocol واستخدامه اختياري بالكامل.

---

## Daemon Protocol Go

Daemon Protocol Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Daemon Protocol، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Daemon Protocol.

1. شغّل الأمر `/connect` في TUI، واختر `Daemon Protocol Go`، ثم انتقل إلى [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
لاستخدام Amazon Bedrock مع Daemon Protocol:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Daemon Protocol غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم Daemon Protocol نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Daemon Protocol
...
لاستخدام Google Vertex AI مع Daemon Protocol:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Daemon Protocol تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Daemon Protocol مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Daemon Protocol. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Daemon Protocol:
...
3. انسخ مفتاح API لاستخدامه في Daemon Protocol.
...
6. **مهم**: قبل استخدام نماذج السحابة في Daemon Protocol، يجب سحب معلومات النموذج محليا:
...
Daemon Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Daemon Protocol. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Daemon Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Daemon Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Daemon Protocol.
...
تسمح حقول `limit` لـ Daemon Protocol بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
