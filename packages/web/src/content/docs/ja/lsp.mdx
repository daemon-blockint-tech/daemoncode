---
title: LSPサーバー
description: Daemon Protocol は LSP サーバーと統合します。
---

Daemon Protocol は Language Server Protocol (LSP) サーバーと統合し、診断を agent へのフィードバックとして使用できます。

---

## 組み込み

Daemon Protocol には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
LSP が有効で Daemon Protocol がファイルを開くと、次のようになります。
...
Daemon Protocol 設定の `lsp` セクションを通じて LSP サーバーを有効化し、カスタマイズできます。
