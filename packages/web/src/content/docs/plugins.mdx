---
title: Plugins
description: Write your own plugins to extend Daemon Protocol.
---

Plugins allow you to extend Daemon Protocol by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify Daemon Protocol's default behavior.
...
Daemon Protocol runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
:::note
If you’re using the Daemon Protocol desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://daemonprotocol.com/docs/sdk) for details.
