...
import type { DaemonCodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@daemon-protocol/sdk/v2"
import type { TuiConfig } from "@daemon-protocol/tui/config"
...
type PromptModel = Parameters<DaemonCodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<DaemonCodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<DaemonCodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<DaemonCodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<DaemonCodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<DaemonCodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<DaemonCodeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<DaemonCodeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<DaemonCodeClient["question"]["reject"]>[0]
