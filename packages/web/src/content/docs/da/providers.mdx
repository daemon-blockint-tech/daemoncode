---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Daemon Protocol.
---

import config from "../../../../config.mjs"
export const console = config.console

Daemon Protocol bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Daemon Protocol-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i Daemon Protocol-konfigurationen.
...
Daemon Zen er en liste over modeller leveret af Daemon Protocol-teamet, som er blevet testet og verificeret til at fungere godt med Daemon Protocol. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Daemon Zen.
:::
...
Det fungerer som alle andre udbydere i Daemon Protocol og er helt valgfrit at bruge.

---

## Daemon Protocol Go

Daemon Protocol Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Daemon Protocol-teamet, som er testet og verificeret til at fungere godt med Daemon Protocol.

1. Kør kommandoen `/connect` i TUI, vælg `Daemon Protocol Go`, og gå til [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
Sådan bruger du Amazon Bedrock med Daemon Protocol:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Daemon Protocol understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Daemon Protocol bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Daemon Protocol
...
Sådan bruger du GitHub Copilot-abonnementet med Daemon Protocol:
...
Sådan bruger du Google Vertex AI med Daemon Protocol:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Daemon Protocol-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Daemon Protocol-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for Daemon Protocol. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Daemon Protocol:
...
3. Kopier API-nøglen til brug i Daemon Protocol.
...
6. **Vigtigt**: Før du bruger skymodeller i Daemon Protocol, skal du hente modelinformationen lokalt:
...
Daemon Zen er en liste over testede og verificerede modeller leveret af Daemon Protocol-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Daemon Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Daemon Zen**.

   ```txt
   /connect
   ```

3. Indtast Daemon Protocol API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Daemon Protocol:
...
`limit`-felterne lader Daemon Protocol forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
