...
description: Usa la configurazione JSON di Daemon Protocol.
---

Puoi configurare Daemon Protocol usando un file di configurazione JSON.

---

## Formato

Daemon Protocol supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
Metti la configurazione globale di Daemon Protocol in `~/.config/opencode/opencode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Quando Daemon Protocol si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Il file di configurazione ha uno schema definito in [**`daemonprotocol.com/config.json`**](https://daemonprotocol.com/config.json).

La configurazione TUI usa [**`daemonprotocol.com/tui.json`**](https://daemonprotocol.com/tui.json).
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Daemon Protocol.
...
Puoi configurare provider e modelli da usare in Daemon Protocol tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Daemon Protocol prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Daemon Protocol fara fallback su `"build"` mostrando un avviso.
...
Daemon Protocol scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono Daemon Protocol con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare Daemon Protocol a usare solo alcuni provider invece di disabilitarli uno per uno.
