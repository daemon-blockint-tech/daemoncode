---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Daemon Protocol.
---

import config from "../../../../config.mjs"
export const console = config.console

Daemon Protocol verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Daemon Protocol-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Daemon Protocol anpassen
config.
...
Daemon Zen ist eine Liste von Modellen, die vom Daemon Protocol-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Daemon Protocol funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Daemon Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Daemon Protocol aus und gehen Sie zu [daemonprotocol.com/auth](https://daemonprotocol.com/auth).
...
Es funktioniert wie jeder andere Anbieter in Daemon Protocol und ist völlig optional.

---

## Daemon Protocol Go

Daemon Protocol Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Daemon Protocol-Team getestet und verifiziert wurden, dass sie gut mit Daemon Protocol funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Daemon Protocol Go` und gehen Sie zu [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
So verwenden Sie Amazon Bedrock mit Daemon Protocol:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Daemon Protocol ausführen:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Daemon Protocol wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Daemon Protocol ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Daemon Protocol-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `Daemon Protocol`, Bereiche: `api`
...
um das Token im Daemon Protocol-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Daemon Protocol verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Daemon Protocol
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Daemon Protocol:
...
So verwenden Sie Google Vertex AI mit Daemon Protocol:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Daemon Protocol ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Daemon Protocol konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede Daemon Protocol-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Daemon Protocol-Konversation als separate Sitzung aufgeführt.
...
Sie können Daemon Protocol für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Daemon Protocol über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Daemon Protocol über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Daemon Protocol konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit Daemon Protocol:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Daemon Protocol.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Daemon Protocol verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Daemon Zen ist eine Liste getesteter und verifizierter Modelle, die vom Daemon Protocol-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Daemon Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Daemon Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Daemon Protocol API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Daemon Protocol-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Daemon Protocol-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Daemon Protocol:
...
Sie können Modelle auch über Ihre Daemon Protocol-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Daemon Protocol verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
Mithilfe der `limit`-Felder kann Daemon Protocol erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Daemon Protocol-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Daemon Protocol-Konfiguration übereinstimmt.
