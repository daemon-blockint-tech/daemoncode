...
Daemon Protocol Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

Go works like any other provider in Daemon Protocol. You subscribe to Daemon Protocol Go and
...
Daemon Protocol Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

Daemon Protocol Go works like any other provider in Daemon Protocol.

1. You sign in to **<a href={console}>Daemon Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Daemon Protocol Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Daemon Protocol Go.
:::
...
Daemon Protocol Go includes the following limits:
...
We created Daemon Protocol Go to:
...
4. Have **no lock-in** by allowing you to use any other provider with Daemon Protocol as well.
