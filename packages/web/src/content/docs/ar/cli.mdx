---
title: CLI
description: أوامر وخيارات CLI في Daemon Protocol.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Daemon Protocol افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
opencode
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Daemon Protocol برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Daemon Protocol.
...
يتضمن CLI الخاص بـ Daemon Protocol أيضا الأوامر التالية.

---

### agent

إدارة وكلاء Daemon Protocol.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Daemon Protocol قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
opencode attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Daemon Protocol تعمل عن بعد. على سبيل المثال:
...
يعتمد Daemon Protocol على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `opencode auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

عند تشغيل Daemon Protocol يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Daemon Protocol.
...
بدء خادم Daemon Protocol بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.
...
عرض جميع جلسات Daemon Protocol.
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Daemon Protocol لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Daemon Protocol.

```bash
opencode import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Daemon Protocol.
...
بدء خادم Daemon Protocol بدون واجهة مع واجهة ويب.

```bash
opencode web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Daemon Protocol عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
إلغاء تثبيت Daemon Protocol وإزالة جميع الملفات ذات الصلة.
...
يمكن تهيئة Daemon Protocol باستخدام متغيرات البيئة.
