...
Det er to typer agenter i Daemon Protocol; primære agenter og underagenter.
...
Daemon Protocol kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Daemon Protocol kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Daemon Protocol kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Hvis ingen temperatur er spesifisert, bruker Daemon Protocol modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Daemon Protocol-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Daemon Protocol-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Daemon Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/daemon-blockint-tech/daemoncode).
:::
