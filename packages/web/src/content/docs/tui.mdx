---
title: TUI
description: Using the Daemon Protocol terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Daemon Protocol provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Daemon Protocol starts the TUI for the current directory.
...
When using the Daemon Protocol TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Daemon Protocol. Allows you to select from available providers and add their API keys.
...
Exit Daemon Protocol. _Aliases_: `/quit`, `/q`
...
- `leader_timeout` - Controls how long Daemon Protocol waits after the leader key. Defaults to `2000`.
