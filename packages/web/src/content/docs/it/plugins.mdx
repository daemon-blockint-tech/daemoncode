---
title: Plugin
description: Scrivi plugin per estendere Daemon Protocol.
---

I plugin ti permettono di estendere Daemon Protocol agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Daemon Protocol.
...
Daemon Protocol esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
:::note
Se usi l'app desktop di Daemon Protocol, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://daemonprotocol.com/docs/sdk) per i dettagli.
