...
代理技能讓 Daemon Protocol 能夠從你的儲存庫或主目錄中發現可重複使用的指令。
...
對於專案本地路徑，Daemon Protocol 會從當前工作目錄向上遍歷，直到到達 git 工作樹根目錄。
...
Daemon Protocol 會在 `skill` 工具描述中列出可用技能。
每個條目包含技能名稱和描述：
