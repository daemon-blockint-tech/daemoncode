---
title: Support ACP
description: Utilisez Daemon Protocol dans n’importe quel éditeur compatible ACP.
---

Daemon Protocol prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser Daemon Protocol via ACP, configurez votre éditeur pour exécuter la commande `opencode acp`.

La commande démarre Daemon Protocol en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
Pour l'ouvrir, utilisez le nouvel agent « Daemon Protocol » dans le sélecteur d'agent AI Chat.
...
Pour utiliser Daemon Protocol comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser Daemon Protocol comme agent ACP pour le chat.
...
Daemon Protocol fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration Daemon Protocol
