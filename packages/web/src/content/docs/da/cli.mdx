---
title: CLI
description: Daemon Protocol CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Daemon Protocol CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opencode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med Daemon Protocol programmatisk.
...
Start Daemon Protocol terminalbrugergrænsefladen.
...
Administrer agenter i Daemon Protocol.
...
Tilslut en terminal til en allerede kørende Daemon Protocol backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern Daemon Protocol backend. For eksempel:
...
Daemon Protocol drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opencode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Når Daemon Protocol starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i Daemon Protocol.
...
Start en hovedløs Daemon Protocol-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.
...
Administrator Daemon Protocol sessionsholder.
...
Liste over alle Daemon Protocol sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine Daemon Protocol-sessioner.
...
Importer sessionsdata fra en JSON fil eller Daemon Protocol del URL.
...
Start en hovedløs Daemon Protocol-server med en webgrænseflade.

```bash
opencode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til Daemon Protocol via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Afinstaller Daemon Protocol og fjern alle relaterede filer.
...
Opdaterer Daemon Protocol til den nyeste version eller en specifik version.
...
Daemon Protocol kan konfigureres ved hjælp af miljøvariabler.
