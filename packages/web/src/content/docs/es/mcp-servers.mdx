...
Puede agregar herramientas externas a Daemon Protocol utilizando el _Model Context Protocol_ o MCP. Daemon Protocol admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Daemon Protocol Config](https://daemonprotocol.com/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Daemon Protocol maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Daemon Protocol:
...
Si el servidor requiere autenticación, Daemon Protocol le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `opencode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Daemon Protocol almacenará los tokens de forma segura en `~/.local/share/opencode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Daemon Protocol, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Daemon Protocol como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Daemon Protocol a su cuenta Sentry.
