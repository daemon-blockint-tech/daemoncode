...
import { resolveThemeVariant } from "@daemon-protocol/ui/theme/resolve"
import type { DesktopTheme } from "@daemon-protocol/ui/theme/types"
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  const failed = (
    ...
    void show(
      "Daemon Protocol failed to load",
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "Daemon Protocol window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("Daemon Protocol is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
