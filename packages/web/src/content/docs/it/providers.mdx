---
title: Provider
description: Usa qualsiasi provider LLM in Daemon Protocol.
---

import config from "../../../../config.mjs"
export const console = config.console

Daemon Protocol usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Daemon Protocol.
...
configurazione di Daemon Protocol.
...
Daemon Zen e una lista di modelli forniti dal team Daemon Protocol, testati e
verificati per funzionare bene con Daemon Protocol. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Daemon Zen.
:::

1. Esegui `/connect` nella TUI, seleziona opencode e vai su [daemonprotocol.com/auth](https://daemonprotocol.com/auth).
...
Funziona come qualsiasi altro provider in Daemon Protocol ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Daemon Protocol:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Daemon Protocol non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Scegli il tuo metodo di autenticazione:
   ...
   3. Nome: `Daemon Protocol`, Scopes: `api`
...
Daemon Protocol usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Daemon Protocol
...
Per usare Google Vertex AI con Daemon Protocol:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Daemon Protocol come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Daemon Protocol elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per Daemon Protocol. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con Daemon Protocol:
...
3. Copia la chiave API da usare in Daemon Protocol.
...
6. **Importante**: Prima di usare modelli cloud in Daemon Protocol, devi scaricare le informazioni del modello localmente:
...
Daemon Zen è una lista di modelli testati e verificati forniti dal team Daemon Protocol. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Daemon Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Daemon Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Daemon Protocol.
...
I campi `limit` permettono a Daemon Protocol di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
