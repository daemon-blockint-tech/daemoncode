...
Daemon Protocol Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Daemon Protocol-u. Pretplatite se na Daemon Protocol Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Daemon Protocol.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Daemon Protocol-om.
:::
...
Daemon Protocol Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Daemon Protocol Go radi kao bilo koji drugi provajder u Daemon Protocol-u.

1. Prijavite se na **<a href={console}>Daemon Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Daemon Protocol Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Daemon Protocol Go.
:::
...
Daemon Protocol Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Daemon Protocol konfiguraciji
...
Napravili smo Daemon Protocol Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Daemon Protocol koristite i bilo kojeg drugog provajdera.
