...
<p align="center">Daemon Protocol je open source AI agent za programiranje.</p>
<p align="center">
...
# Package manageri
npm i -g daemoncode@latest        # ili bun/pnpm/yarn
scoop install daemoncode             # Windows
choco install daemoncode             # Windows
brew install daemon-blockint-tech/tap/daemoncode # macOS i Linux (preporučeno, uvijek ažurno)
brew install daemoncode              # macOS i Linux (zvanična brew formula, rjeđe se ažurira)
sudo pacman -S daemoncode            # Arch Linux (Stable)
paru -S daemoncode-bin               # Arch Linux (Latest from AUR)
mise use -g daemoncode               # Bilo koji OS
nix run nixpkgs#daemoncode           # ili github:daemon-blockint-tech/daemoncode za najnoviji dev branch
```
...
Daemon Protocol je dostupan i kao desktop aplikacija. Preuzmi je direktno sa [stranice izdanja](https://github.com/daemon-blockint-tech/daemoncode/releases) ili sa [daemonprotocol.com/download](https://daemonprotocol.com/download).
...
Daemon Protocol uključuje dva ugrađena agenta između kojih možeš prebacivati tasterom `Tab`.
...
Saznaj više o [agentima](https://daemonprotocol.com/docs/agents).

### Dokumentacija

Za više informacija o konfiguraciji Daemon Protocol-a, [**pogledaj dokumentaciju**](https://daemonprotocol.com/docs).

### Doprinosi

Ako želiš doprinositi Daemon Protocol-u, pročitaj [upute za doprinošenje](./CONTRIBUTING.md) prije slanja pull requesta.

### Gradnja na Daemon Protocol-u

Ako radiš na projektu koji je povezan s Daemon Protocol-om i koristi "opencode" kao dio naziva, npr. "opencode-dashboard" ili "opencode-mobile", dodaj napomenu u svoj README da projekat nije napravio Daemon Protocol tim i da nije povezan s nama.
