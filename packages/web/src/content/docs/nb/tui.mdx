...
Daemon Protocol gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Daemon Protocol starter TUI for gjeldende katalog.
...
Når du bruker Daemon Protocol TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Daemon Protocol. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt Daemon Protocol. _Aliaser_: `/quit`, `/q`
...
- `leader_timeout` - Styrer hvor lenge Daemon Protocol venter etter leader key. Standard er `2000`.
