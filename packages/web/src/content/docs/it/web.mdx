---
title: Web
description: Usa Daemon Protocol nel browser.
---

Daemon Protocol puo funzionare come applicazione web nel browser, offrendo la stessa potente esperienza di coding AI senza bisogno di un terminale.

![Daemon Protocol Web - Nuova sessione](../../../assets/web/web-homepage-new-session.png)
...
Questo avvia un server locale su `127.0.0.1` con una porta disponibile scelta a caso e apre automaticamente Daemon Protocol nel browser predefinito.
...
Per impostazione predefinita, Daemon Protocol sceglie una porta disponibile. Puoi specificare una porta:
...
Per impostazione predefinita, il server si lega a `127.0.0.1` (solo localhost). Per rendere Daemon Protocol accessibile sulla rete:

```bash
opencode web --hostname 0.0.0.0
```

Usando `0.0.0.0`, Daemon Protocol mostrera sia l'indirizzo locale sia quello di rete:
...
Una volta avviata, l'interfaccia web ti da accesso alle tue sessioni Daemon Protocol.

### Sessioni

Visualizza e gestisci le sessioni dalla homepage. Puoi vedere le sessioni attive e avviarne di nuove.

![Daemon Protocol Web - Sessione attiva](../../../assets/web/web-homepage-active-session.png)

### Stato del server

Fai clic su "See Servers" per vedere i server connessi e il loro stato.

![Daemon Protocol Web - See Servers](../../../assets/web/web-homepage-see-servers.png)
