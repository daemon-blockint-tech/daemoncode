---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Daemon Protocol zu erweitern.
---

Mit Plugins können Sie Daemon Protocol erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Daemon Protocol zu ändern.
...
Daemon Protocol führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Daemon Protocol-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Daemon Protocol erweitern können.
...
:::note
Wenn Sie die Daemon Protocol-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Daemon Protocol `.env`-Dateien liest:
...
Plugins können Daemon Protocol auch benutzerdefinierte Tools hinzufügen:

```ts title=".opencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@daemon-protocol/plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Daemon Protocol aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Daemon Protocol zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://daemonprotocol.com/docs/sdk).
