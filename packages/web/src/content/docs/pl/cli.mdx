---
title: CLI
description: Opcje i polecenia wiersza poleceń Daemon Protocol.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI Daemon Protocol domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
opencode
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać Daemon Protocol programowo.
...
Uruchamia interfejs terminalowy (TUI) Daemon Protocol.
...
Interfejs CLI Daemon Protocol zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami Daemon Protocol.
...
Dołącz terminal do już działającego serwera backendu Daemon Protocol uruchomionego za pomocą `serve` lub `web`.

```bash
opencode attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem Daemon Protocol. Na przykład:
...
Daemon Protocol integruje się z wieloma dostawcami. Możesz użyć `opencode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Podczas uruchamiania Daemon Protocol ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w Daemon Protocol.
...
Uruchom Daemon Protocol w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera Daemon Protocol (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `opencode`) |
...
Uruchom serwer Daemon Protocol (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
opencode serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API Daemon Protocol bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Zarządzaj sesjami Daemon Protocol.
...
Lista wszystkich sesji Daemon Protocol.
...
Pokaż statystyki wykorzystania tokenów i sesji Daemon Protocol.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania Daemon Protocol.
...
Uruchom serwer Daemon Protocol z interfejsem internetowym.

```bash
opencode web
```

Uruchamia to serwer HTTP i udostępnia Daemon Protocol przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Odinstaluj Daemon Protocol i usuń wszystkie powiązane pliki.
...
Aktualizuj Daemon Protocol do najnowszej lub określonej wersji.
...
Interfejs CLI Daemon Protocol przyjmuje następujące flagi globalne dla każdego polecenia.
...
Daemon Protocol można skonfigurować za pomocą zmiennych środowiskowych.
