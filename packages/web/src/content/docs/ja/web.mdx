---
title: ウェブ
description: ブラウザで Daemon Protocol を使用する。
---

Daemon Protocol はブラウザで Web アプリケーションとして実行でき、ターミナルを必要とせずに同じ強力な AI コーディングエクスペリエンスを提供します。

![Daemon Protocol Web - 新しいセッション](../../../assets/web/web-homepage-new-session.png)
...
これにより、利用可能なランダムなポートを使用して `127.0.0.1` でローカルサーバーが起動され、デフォルトのブラウザで Daemon Protocol が自動的に開きます。
...
デフォルトでは、Daemon Protocol は使用可能なポートを選択します。ポートを指定できます。
...
デフォルトでは、サーバーは `127.0.0.1` (localhost のみ) にバインドされます。ネットワーク上で Daemon Protocol にアクセスできるようにするには:

```bash
opencode web --hostname 0.0.0.0
```

`0.0.0.0` を使用すると、Daemon Protocol はローカルアドレスとネットワークアドレスの両方を表示します。
...
開始すると、Web インターフェイスから Daemon Protocol セッションにアクセスできるようになります。

### セッション

ホームページからセッションを表示および管理します。アクティブなセッションを確認したり、新しいセッションを開始したりできます。

![Daemon Protocol Web - アクティブなセッション](../../../assets/web/web-homepage-active-session.png)

### サーバーステータス

「サーバーを表示」をクリックすると、接続されているサーバーとそのステータスが表示されます。

![Daemon Protocol Web - サーバー](../../../assets/web/web-homepage-see-servers.png)
