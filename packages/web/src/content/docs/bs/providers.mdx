---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Daemon Protocol.
---

import config from "../../../../config.mjs"
export const console = config.console

Daemon Protocol koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Daemon Protocol konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Daemon Protocol
config.
...
Daemon Zen je lista modela koje je obezbedio Daemon Protocol tim koji su bili
testirano i potvrđeno da dobro radi sa Daemon Protocol. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Daemon Zen.
:::
...
Radi kao i svaki drugi provajder u Daemon Protocol i potpuno je opcionalan za korištenje.

---

## Daemon Protocol Go

Daemon Protocol Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Daemon Protocol tim i koji su testirani i verificirani da dobro rade s Daemon Protocol-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Daemon Protocol Go` i idite na [daemonprotocol.com/auth](https://daemonprotocol.com/zen).
...
Da biste koristili Amazon Bedrock s Daemon Protocol:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Daemon Protocol.
:::
...
3.  Naziv: `Daemon Protocol`, opseg: `api`
...
Daemon Protocol koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Daemon Protocol
...
Za korištenje Google Vertex AI s Daemon Protocol:
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Daemon Protocol razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za Daemon Protocol. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Daemon Protocol:
...
3. Kopirajte API ključ za korištenje u Daemon Protocol.
...
6. **Važno**: Prije upotrebe modela oblaka u Daemon Protocol, morate lokalno povući informacije o modelu:
...
Daemon Zen je lista testiranih i verifikovanih modela koju je obezbedio Daemon Protocol tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Daemon Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Daemon Zen**.

```txt
   /connect
```

3. Unesite svoj Daemon Protocol API ključ.
...
Polja `limit` omogućavaju Daemon Protocol da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite Daemon Protocol konfiguraciju i:
