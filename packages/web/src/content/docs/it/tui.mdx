---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Daemon Protocol.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Daemon Protocol fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Daemon Protocol avvia la TUI per la directory corrente.
...
Quando usi la TUI di Daemon Protocol, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Daemon Protocol. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Daemon Protocol. _Alias_: `/quit`, `/q`
...
- `leader_timeout` - Controlla per quanto tempo Daemon Protocol attende dopo il leader key. Default: `2000`.
