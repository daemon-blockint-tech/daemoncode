...
エージェントスキルにより、Daemon Protocol はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
プロジェクトのローカルパスの場合、Daemon Protocol は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
...
Daemon Protocol では、`skill` ツールの説明に利用可能なスキルがリストされています。
