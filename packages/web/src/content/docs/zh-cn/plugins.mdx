---
title: 插件
description: 编写自己的插件来扩展 Daemon Protocol。
---

插件允许你通过挂钩各种事件和自定义行为来扩展 Daemon Protocol。你可以创建插件来添加新功能、集成外部服务，或修改 Daemon Protocol 的默认行为。
...
Daemon Protocol 会在启动时运行 `bun install` 来安装这些依赖项。之后你的插件和工具就可以导入它们了。
...
- `directory`：当前工作目录。
- `worktree`：git 工作树路径。
- `client`：用于与 AI 交互的 Daemon Protocol SDK 客户端。
...
以下是一些可用于扩展 Daemon Protocol 的插件示例。
...
:::note
如果你使用 Daemon Protocol 桌面应用，它可以在响应就绪或会话出错时自动发送系统通知。
:::
...
插件还可以为 Daemon Protocol 添加自定义工具：
...
`tool` 辅助函数用于创建 Daemon Protocol 可调用的自定义工具。它接受一个 Zod schema 函数，并返回一个工具定义，包含：

- `description`：工具的功能描述
- `args`：工具参数的 Zod schema
- `execute`：工具被调用时执行的函数

你的自定义工具将与内置工具一起在 Daemon Protocol 中可用。
...
日志级别：`debug`、`info`、`warn`、`error`。详情请参阅 [SDK 文档](https://daemonprotocol.com/docs/sdk)。
