...
Puoi aggiungere strumenti esterni a Daemon Protocol usando il _Model Context Protocol_, o MCP. Daemon Protocol supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Daemon Protocol Config](https://daemonprotocol.com/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Daemon Protocol gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Daemon Protocol:
...
Se il server richiede autenticazione, Daemon Protocol ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `opencode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Daemon Protocol memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Daemon Protocol insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Daemon Protocol come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Daemon Protocol al tuo account Sentry.
